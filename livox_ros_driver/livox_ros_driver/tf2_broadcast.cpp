...
int main(int argc, char **argv)
{
    ...
    if (argc != 9)
    {
        ROS_ERROR("Invalid number of parameters\nusage: static_tf2_broadcaster child_frame_name parent_frame_name x y z roll pitch yaw");
        return -1;
    }
    ...
    static_parent_name = argv[2];
    static tf2_ros::StaticTransformBroadcaster static_broadcaster;
    // Create a TransformStamped message
    geometry_msgs::TransformStamped transformStamped;
    transformStamped.header.stamp = ros::Time::now();    // Set the timestamp
    transformStamped.header.frame_id = static_parent_name;          // Parent frame ID
